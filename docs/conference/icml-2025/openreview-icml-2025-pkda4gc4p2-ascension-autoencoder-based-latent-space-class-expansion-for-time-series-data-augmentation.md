...
title_zh: "ASCENSION: 基于自动编码器的潜在空间类扩展用于时间序列数据增广"
...
score: 6.0
evidence: VAE潜在空间用于时间序列增广，可迁移至EEG潜在表示
tldr: 现有时间序列数据增广方法难以跨领域一致有效。本文提出ASCENSION，利用变分自编码器的潜在空间概率结构进行类扩展，实现高效的数据增广。实验在ECG、电力等多个领域验证其有效性。该方法为EEG信号潜在空间建模和通道补全提供了可迁移的生成框架。
...
motivation: 时间序列数据增广面临跨数据集一致性差的问题。
method: 提出基于VAE潜在空间类扩展的生成式增广方法。
result: 在多种时间序列数据集上表现优于现有方法。
conclusion: ASCENSION为时间序列增广提供了新范式，可应用于EEG领域。
---
