...
title_zh: 基于流匹配的少试次神经适应与稳定潜在动力学
authors: "Puli Wang, Yu Qi, Yueming Wang, Gang Pan"
date: 2025-05-01
pdf: "https://openreview.net/pdf?id=nKJEAQ6JCY"
tags: ["query:eeg-latent"]
score: 6.0
evidence: 使用流匹配对稳定潜在动力学进行BCI神经适应
tldr: 该文针对BCI中因神经信号非平稳性导致的跨天性能退化问题，提出基于流匹配的分布对齐框架（FDA），通过建模稳定潜在动力学实现少试次神经适应。实验结果表明，在仅有少量校准试次的情况下，FDA能够显著提升跨天解码性能，且优于现有对齐方法。该工作展示了流匹配模型在脑信号潜在空间对齐中的潜力，为BCI实际部署提供了高效适应方案。
...
motivation: BCI中神经信号非平稳性导致跨天性能下降，现有对齐方法在少试次时不稳定。
method: 提出基于流匹配的分布对齐框架，对神经信号的潜在动力学进行建模和对齐。
result: 在少量试次下，FDA能有效对齐跨天神经信号，提升解码准确率。
conclusion: 流匹配模型为少试次BCI适应提供了一种稳定高效的潜在空间对齐方法。
---
