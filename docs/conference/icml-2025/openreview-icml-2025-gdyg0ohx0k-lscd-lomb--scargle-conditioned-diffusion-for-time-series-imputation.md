...
score: 6.0
evidence: 扩散模型用于时间序列插补可应用于脑电图通道补全
tldr: 本文提出LSCD，一种将可微Lomb-Scargle层集成到基于分数的扩散模型中的时间序列插补方法。该方法针对不规则采样数据，通过频谱条件实现更准确的缺失值恢复。虽然论文未直接应用于脑电图，但其扩散模型框架和频谱分析技术可直接迁移至脑电图缺失通道补全任务，为潜在空间生成模型在脑电图信号重建中的应用提供了方法论桥梁。
...
motivation: 现有时间序列插补方法在处理不规则采样数据时依赖FFT，需先插值导致频谱失真。
method: 提出可微Lomb-Scargle层计算不规则采样数据的功率谱，并将其作为条件输入扩散模型进行插补。
result: 在合成和真实基准上，该方法比纯时域基线更准确地恢复缺失数据，同时保留频谱特性。
conclusion: 融合频谱条件扩散模型可有效提升时间序列插补的精度和鲁棒性。
---
