...
tags: ["query:eeg"]
score: 7.0
evidence: 包含EEG信号处理的多模态基础模型
tldr: 该论文提出NormWear，一个针对可穿戴传感数据的预训练基础模型，在包括PPG、ECG、EEG、GSR和IMU等多种公开数据集上进行训练。通过提取通用且信息丰富的表示，NormWear在多个下游任务中表现出色，可有效处理EEG信号的特征提取，为EEG信号分析提供了一种强大的基础模型。
...
motivation: 可穿戴生理信号模式多样，缺乏通用表示方法。
method: 提出NormWear，在多种生理信号上预训练，学习通用波形表示。
result: 在多个评估任务中表现优异，验证了模型泛化能力。
conclusion: NormWear可作为EEG等生理信号分析的基础工具。
---
