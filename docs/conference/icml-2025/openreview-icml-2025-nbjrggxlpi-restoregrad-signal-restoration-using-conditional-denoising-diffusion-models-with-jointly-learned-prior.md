...
title_zh: RestoreGrad：基于联合学习先验的条件去噪扩散模型的信号恢复
...
score: 6.0
evidence: 条件扩散模型用于信号恢复可应用于脑电图重建
tldr: 本文提出RestoreGrad，一种条件去噪扩散概率模型，通过联合学习一个更具信息量的先验分布来替代标准高斯先验，从而提升信号恢复质量。该方法在降质观测信号条件下重建干净信号，其框架可直接迁移至脑电图信号重建与缺失通道补全场景。实验表明，联合学习先验显著优于传统条件扩散模型，为脑电图潜在空间生成提供了有效工具。
...
motivation: 现有条件扩散模型使用标准高斯先验丢弃了降质信号中的有用信息。
method: 提出与扩散模型联合学习的可训练先验，替代固定高斯先验，改进条件去噪过程。
result: 在多种信号恢复任务上，联合学习先验比标准条件扩散模型取得更优的性能。
conclusion: 更丰富的先验分布能够显著提升条件扩散模型在信号恢复中的效果。
---
