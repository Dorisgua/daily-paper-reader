...
title_zh: EEG-语言预训练实现高度标签高效的临床表型分析
authors: "Sam Gijsen, Kerstin Ritter"
date: 2025-05-01
pdf: "https://openreview.net/pdf?id=yaI2ZYFmeD"
tags: ["query:eeg-latent"]
score: 9.0
evidence: EEG-语言预训练结合多模态对齐用于临床表型分析
tldr: 多模态语言模型在脑功能数据领域尚未探索。本文开创性地提出EEG-语言模型（ELM），在15000份EEG和临床报告上训练。通过多模态对齐及时序裁剪、文本分割，结合多实例学习解决不匹配问题。相比仅用EEG的模型，在四项临床评估中显著提升，并首次实现零样本分类和信号/报告检索。直接贡献于EEG潜在空间表征学习。
...
motivation: 多模态语言模型在脑功能数据临床表型分析中的应用尚属空白。
method: 提出EEG-语言预训练模型，结合多模态对齐、时序裁剪、文本分割和多实例学习。
result: 在四项临床评估中显著优于仅EEG模型，实现零样本分类和检索。
conclusion: ELM证明了EEG-语言联合预训练的巨大潜力，为EEG潜在表征研究奠定了重要基础。
---
