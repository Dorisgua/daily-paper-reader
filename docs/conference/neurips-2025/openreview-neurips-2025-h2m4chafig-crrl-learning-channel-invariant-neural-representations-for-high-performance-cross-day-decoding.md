...
title_zh: CRRL：学习通道不变神经表示以实现高性能跨日解码
authors: "Xianhan Tan, Binli Luo, Yu Qi, Yueming Wang"
date: 2025-09-18
pdf: "https://openreview.net/pdf?id=H2m4chAfig"
tags: ["query:eeg-latent"]
score: 4.0
evidence: 学习通道不变神经表示用于跨日解码，与EEG潜在空间相关
tldr: 该论文针对脑机接口跨日性能不稳定的问题，提出CRRL方法学习通道不变神经表示。通过通道重排模块消除电极位移和神经元死亡导致的通道变异性，从而稳定解码性能。方法虽未明确针对EEG，但处理的是多通道神经信号，与EEG latent空间学习高度相关。在多个动物数据集上验证了跨日解码精度的提升。
...
motivation: 脑机接口信号跨日不稳定，通道级变异性影响解码性能。
method: 提出通道重排模块学习通道不变表示，减少跨日信号漂移的影响。
result: 在多个跨日神经数据集上显著提升了解码稳定性。
conclusion: 该方法为多通道神经信号的潜在表示学习提供了有效思路，可推广到EEG场景。
---
