...
title_zh: CSBrain：用于脑电图解码的跨尺度时空基础模型
...
tags: ["query:eeg"]
score: 8.0
evidence: 具有跨尺度时空结构的脑电图解码基础模型
tldr: 现有EEG基础模型忽视神经活动的跨尺度时空结构，CSBrain针对此问题提出一种捕捉不同时空尺度模式（从快速神经激活到慢变节律）的统一架构。通过大规模预训练，在多种脑解码任务（如认知、情绪识别、临床诊断）上取得优异性能，提升了EEG解码的泛化能力，为脑机接口和神经科学提供强大基础模型。
...
motivation: 现有EEG基础模型继承NLP和视觉的尺度无关密集建模，忽视了神经活动内在的跨尺度时空结构。
method: 提出CSBrain，一种捕捉不同时间空间尺度的统一架构，采用跨尺度机制处理从短暂激活到慢变节律的模式。
result: 在大规模EEG数据上预训练，在多个下游任务中显著提升解码性能，证明了跨尺度建模的有效性。
conclusion: 为EEG解码提供了更符合神经生理特性的基础模型，推动脑机接口和临床诊断应用。
---
