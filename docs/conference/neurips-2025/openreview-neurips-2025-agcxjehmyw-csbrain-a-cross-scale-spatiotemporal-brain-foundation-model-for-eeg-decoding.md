...
title_zh: CSBrain：用于EEG解码的跨尺度时空脑基础模型
...
tags: ["query:eeg"]
score: 8.0
evidence: 用于脑机接口的EEG解码
tldr: 现有EEG基础模型采用尺度无关的密集建模，忽视了神经活动的跨尺度时空特性。本文提出CSBrain，一个跨尺度时空脑基础模型，通过捕获不同时间和空间尺度的神经模式来提升EEG解码性能。实验表明该模型在多个EEG解码任务上优于现有方法，为脑机接口和临床诊断提供了更通用的EEG表示学习框架。
...
motivation: 现有EEG基础模型采用尺度无关的密集建模，忽略了神经活动的跨尺度时空特性。
method: 提出跨尺度时空脑基础模型CSBrain，捕获不同尺度的时空神经模式。
result: 在多个EEG解码任务上优于现有方法，提升泛化能力。
conclusion: CSBrain通过跨尺度建模为通用EEG表示学习提供了新范式。
---
