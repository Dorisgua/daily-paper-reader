...
title_zh: THD-BAR：用于EEG通用表示的拓扑层次派生脑自回归建模
...
score: 8.0
evidence: 通过拓扑层次自回归建模学习通用EEG表示
tldr: 现有EEG自回归模型仅依赖时序，无法捕获脑动态拓扑。本文提出THD-BAR，通过拓扑层次派生脑自回归建模，利用分层空间拓扑结构学习通用EEG表示。实验证明该方法在多个EEG任务上取得优异性能，为EEG潜在空间学习提供了新思路。
...
motivation: 现有EEG自回归模型仅使用时序，忽略了脑活动的空间拓扑动态。
method: 提出拓扑层次派生脑自回归建模，融合分层空间拓扑与时间建模。
result: 在多个EEG基准任务上取得最先进性能。
conclusion: THD-BAR有效学习通用EEG表示，可应用于下游任务。
---
