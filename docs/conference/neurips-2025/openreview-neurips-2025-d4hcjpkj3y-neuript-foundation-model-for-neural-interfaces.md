...
score: 8.0
evidence: 用于神经接口的基础模型，学习EEG潜在表示
tldr: 该论文提出NeurIPT，一个专为多样EEG神经接口设计的Transformer基础模型。通过捕获EEG信号中的同质和异质时空特性，学习到鲁棒的潜在空间表示，在跨主体、跨任务设置下显著提升解码性能。
...
motivation: EEG基础模型面临跨主体、跨任务和不同电极配置的巨大变异性。
method: 采用预训练Transformer捕获同质和异质时空特征，学习通用表示。
result: 在多个BCI基准数据集上超越了专门模型。
conclusion: 为通用EEG神经接口提供了强大的基础模型。
---
