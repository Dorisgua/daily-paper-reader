...
tags: ["query:eeg"]
score: 8.0
evidence: 基于EEG的神经接口基础模型
tldr: 本文针对EEG数据跨被试、跨任务和跨电极配置的变异性，提出NeurIPT基础模型。该模型采用预训练Transformer架构，同时学习EEG信号的同质和异质时空特征。在多个BCI基准数据集上，NeurIPT显著提升了神经解码性能，展示了基础模型在通用脑电分析中的潜力。
...
motivation: EEG基础模型面临跨域变异和电极配置差异的挑战。
method: 设计预训练Transformer，同时捕获同质和异质时空特征，适应多种电极布局。
result: 在多个BCI任务上达到最先进性能，验证了模型泛化能力。
conclusion: NeurIPT为EEG通用解码提供了一个可扩展的基础框架。
---
