...
title_zh: 多数据集联合预训练情感EEG实现可泛化情感计算
authors: "Qingzhu Zhang, Jiani Zhong, Li ZongSheng, Xinke Shen, Quanying Liu"
date: 2025-09-18
pdf: "https://openreview.net/pdf?id=xaxuzubN31"
tags: ["query:eeg"]
score: 7.0
evidence: 情感EEG预训练用于脑机接口
tldr: 现有EEG模型在情感识别等复杂任务上泛化能力不足。本文提出一种针对情感EEG的多数据集联合预训练框架，通过跨数据集协方差对齐损失对齐二阶统计特性，解决跨数据集分布偏移和个体差异问题。实验表明该方法能实现鲁棒的跨数据集情感识别，无需额外标定。该工作为EEG情感计算提供了可迁移的预训练范式。
...
motivation: 现有任务通用EEG预训练模型在情感识别等复杂任务上表现不佳，缺乏任务特定预训练方法。
method: 提出多数据集联合预训练框架，引入跨数据集协方差对齐损失对齐二阶统计特性。
result: 在跨数据集情感识别上取得鲁棒泛化性能，无需额外标定。
conclusion: 任务特定多数据集预训练能有效提升EEG情感识别的跨数据集泛化能力。
---
