...
title_zh: 情感EEG多数据集联合预训练实现可泛化的情感计算
authors: "Qingzhu Zhang, Jiani Zhong, Li ZongSheng, Xinke Shen, Quanying Liu"
date: 2025-09-18
pdf: "https://openreview.net/pdf?id=xaxuzubN31"
tags: ["query:eeg"]
score: 7.0
evidence: 多数据集联合预训练情感EEG以实现可泛化的情感计算
tldr: 该论文针对情感EEG任务通用预训练模型与下游任务不匹配的问题，提出任务特定的多数据集联合预训练框架。通过引入跨数据集协方差对齐损失，对齐不同数据集间的二阶统计属性，从而在无需大量标注的情况下实现跨数据集鲁棒泛化。在多个情感EEG数据集上验证了其有效性，推动了情感计算领域通用预训练模型的发展。
...
motivation: 现有EEG预训练模型在情感识别任务上泛化能力差。
method: 设计任务特定预训练框架，使用协方差对齐损失跨数据集对齐。
result: 在多个数据集上实现鲁棒的跨数据及跨被试情感识别。
conclusion: 为情感EEG提供了可泛化的预训练范式。
---
