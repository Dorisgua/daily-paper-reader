...
title_zh: BrainOmni：统一的脑电图和脑磁图基础模型
...
tags: ["query:eeg"]
score: 7.0
evidence: 统一EEG和MEG的基础模型，使用BrainTokenizer进行时空标记化
tldr: 脑电图和脑磁图信号在传感器配置上存在差异，现有模型常局限于特定数据集。本文提出BrainOmni，首个统一EEG和MEG的基础模型，通过BrainTokenizer将时空脑活动量化为离散标记，实现跨数据集泛化。实验表明该模型在多个EEG/MEG基准上表现优异，为脑信号分析提供了统一框架。
...
motivation: 现有EEG/MEG模型受限于模态和数据集特异性，缺乏统一框架。
method: 提出BrainOmni，包含BrainTokenizer将时空脑活动量化为离散标记，实现跨模态统一表征。
result: 在多个EEG/MEG数据集上验证了跨数据集泛化能力，性能优于专用模型。
conclusion: BrainOmni作为首个统一脑信号基础模型，推动了EEG/MEG分析的标准化和可扩展性。
---
