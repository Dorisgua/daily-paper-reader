...
title_zh: NEED：面向EEG信号视频和图像重建的跨被试与跨任务泛化
...
score: 8.0
evidence: 使用深度学习从EEG信号重建视觉刺激，解决跨被试泛化问题
tldr: 该论文提出NEED框架，首次实现EEG视觉重建的零样本跨被试和跨任务泛化。通过个体适应模块解决被试差异，并采用多尺度空间编码提升分辨率。在多个视觉重建任务上验证了有效性，性能优于现有方法，推动了EEG解码向实用化迈进。该工作克服了传统模型泛化性差的瓶颈，为脑机接口实际应用奠定了基础。
...
motivation: 现有EEG视觉解码模型泛化性差且局限于特定任务，亟需统一的跨被试、跨任务框架。
method: 提出个体适应模块和多尺度空间编码，结合预训练和零样本迁移，实现跨被试泛化。
result: 在多个数据集上实现视频和图像重建的零样本泛化，性能优于现有方法。
conclusion: NEED为EEG视觉解码的实用化提供了通用框架，可推广至其他脑信号解码任务。
---
