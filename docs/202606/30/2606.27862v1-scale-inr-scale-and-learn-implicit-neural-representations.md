...
title_zh: ScaLe-INR：尺度化与学习隐式神经表示
...
tldr: 隐式神经表示受频谱偏差和信息串扰困扰，单网络难以同时学习多尺度信号。本文提出ScaLe-INR多分支架构，通过方向坐标缩放匹配信号频谱，并设计方向边缘引导损失约束高频分支作为局部边缘滤波器，从而消除串扰。在图像重建、去噪、音频和3D重建任务上分别达到+5.16 dB、+0.65 dB、50.02 dB和0.999 IOU，全面超越现有方法。该工作为多尺度连续信号建模提供了高效解耦方案。
source: arxiv
...
motivation: 现有INR因频谱偏差和信息串扰，单网络无法高质量建模多尺度信号，高频更新破坏低频结构。
method: 提出ScaLe-INR，利用方向坐标缩放扩展表示带宽，并引入方向边缘引导损失强制高频分支充当边缘滤波器，实现功能解耦。
result: 图像重建+5.16 dB，图像去噪+0.65 dB，音频重建50.02 dB，3D重建IOU 0.999，均超越SOTA。
conclusion: ScaLe-INR有效消除频谱串扰，加速收敛，实现多尺度信号的高保真重建，性能显著优于现有方法。
---

## 摘要
隐式神经表示（INR）由多层感知机参数化，擅长建模连续信号。然而，一个关键挑战仍然存在：INR从根本上受到频谱偏差和信息串扰的影响。当单个网络试图捕捉多尺度现象时，高频权重更新会破坏性地干扰底层低频结构近似。我们引入了尺度与学习INR（ScaLe-INR），一种新颖的多分支架构，通过将信号的频谱与INR的最佳工作区域显式匹配来解决这些限制。借鉴傅里叶逆缩放定理，我们证明了应用方向坐标缩放可以沿着特定空间轴扩展网络的表示带宽。为了在数学上实现功能解缠并最小化分支间的任务特定信息泄漏，我们提出了一种方向边缘引导损失，这是一种从真实梯度导出的空间条件稀疏先验。通过约束高频分支作为严格的、局部的边缘滤波器，ScaLe-INR消除了频谱串扰，加速了收敛，并在复杂的多尺度拓扑上实现了高保真信号重建。我们在各种重建和逆任务上评估了ScaLe-INR，证明了相对于现有最先进（SOTA）方法的显著性能提升。所提出的架构在图像重建上比最近的基线提高了+5.16 dB，在图像去噪上提高了+0.65 dB。此外，它在音频重建上达到了令人印象深刻的50.02 dB，在3D重建上达到了0.999 IOU（交并比），击败了所有SOTA模型。
