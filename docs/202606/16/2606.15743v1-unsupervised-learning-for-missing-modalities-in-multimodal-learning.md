...
score: 6.0
evidence: 缺失模态的无监督填补方法，可应用于EEG通道填补
tldr: "多模态学习中模态缺失是常见挑战。本文提出无监督框架UL4M4，通过模态特定归一化和部分模态距离度量对不完整观测进行公平聚类，并利用聚类中心迭代贪心插补缺失特征。在超过50%模态缺失的挑战性配置下，首次实现一致的F1-Micro分数高于0.7。该轻量级方法任务无关，易于集成，显著优于现有基线。"
source: arxiv
...
motivation: 解决多模态学习中任意模态缺失问题，无需依赖完整数据或下游任务。
method: 无监督聚类结合贪心插补，使用模态特定归一化和部分模态距离度量处理不完整观测。
result: "在超过50%模态缺失时F1-Micro>0.7，性能稳定且超越基线。"
conclusion: 提出灵活、任务无关的缺失模态插补框架，鲁棒性强，易于集成到任何融合架构。
---

## 摘要
本文通过引入多模态学习中缺失模态的无监督学习（UL4M4），解决了多模态学习中的缺失模态挑战。UL4M4是一个灵活的框架，在监督预测之前以任务无关的方式填补缺失的特征嵌入。我们提出了模态特定的归一化以及一种新的部分模态距离度量，以实现对不完整观测的公平聚类，捕捉跨模态结构，同时在不同维度和模态数量下保持尺度不变性。来自无监督阶段的聚类中心指导训练或推理过程中任何缺失模态的迭代贪婪填补过程，支持任意数量的模态和每个样本的任意缺失模式。填补模块轻量级，使用冻结的编码器，并与下游任务解耦，可以轻松集成到任何融合/预测架构中。在多样且高度不完整场景下的广泛实验证明了UL4M4的鲁棒性，据我们所知，即使超过50%的模态槽缺失，它在具有挑战性的缺失配置上首次一致地实现了高于0.7的F1-Micro分数。结果在不同聚类大小下也保持稳定，并显著优于最先进的基线方法。代码可在此处获取：https://github.com/h-ismkhan/Multimodal-Learning-with-Missing-Modalities-via-Unsupervised-Learning。
